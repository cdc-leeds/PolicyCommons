...
<div class="{cycle values="evenrow,oddrow"}">
	...
	<div>
		<span class="method-title">{$functions[func].function_name|lower}</span>
	</div> 

	{include file="docblock.tpl" sdesc=$functions[func].sdesc desc=$functions[func].desc params=$functions[func].params function=false}
	...
	{if $functions[func].params}
		...
		{section name=params loop=$functions[func].params}
			<li>
				...
				<span class="var-name">{$functions[func].params[params].var|replace:'$':''}</span>{if $functions[func].params[params].data}<span class="var-description"> {$functions[func].params[params].data}</span>{/if}
			</li>
		{/section}
		</ul>
	{/if}
	
	{if $functions[func].tags}
		<ul class="tags">
			{section name=tags loop=$functions[func].tags}
			<li><span class="field">{$functions[func].tags[tags].keyword}:</span> 
				{$functions[func].function_return}
				{if $functions[func].tags[tags].data}
					{$functions[func].tags[tags].data}
				{/if}
			</li>
			{/section}
		</ul>
